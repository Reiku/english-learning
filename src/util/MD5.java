        ...
        public class MD5 {
            ...
            /*public static void main(String[] args) {
                if (args.length == 0) {
                    ...
                    for (String element : args) {
                        try {
                            System.out.println(MD5.getHashString(new File(
                                    element))
                                    + " " + element);
                        } catch (IOException x) {
            ...
            public byte[] getHash() {
                if (!finalState.valid) {
                    ...
                    int padlen = (leftOver < 56) ? (56 - leftOver)
                            : (120 - leftOver);
                    // add the padding
             ...
             * @since ostermillerutils 1.00.00
             */
            private void update(MD5State state, byte buffer[], int offset,
                    int length) {
                ...
                int i = 0;
                if (length >= partlen) {
                    System.arraycopy(buffer, offset, state.buffer, index,
                            partlen);
             ...
             * @since ostermillerutils 1.00.00
             */
            private static final byte padding[] = { (byte) 0x80, 0, 0, 0, 0, 0,
            ...
            private class MD5State {
                ...
                private void copy(MD5State from) {
                    System.arraycopy(from.buffer, 0, this .buffer, 0,
                            this .buffer.length);
                    System.arraycopy(from.state, 0, this .state, 0,
                            this .state.length);
            ...
            private int[] decode(byte buffer[], int len, int offset) {
                int i, j;
                for (i = j = 0; j < len; i++, j += 4) {
                    decodeBuffer[i] = ((buffer[j + offset] & 0xff))
